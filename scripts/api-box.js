...
///////////////////////////////////////////////////////
// Register {% apibox name } tag with HEXO
///////////////////////////////////////////////////////
...
var apiData = function (options) {
  ...
  // add a field for each param that indicates the tab depth
  _.each(root.params, (p, i) => root.params[i].indentClass = 'tab'+(p.name.match(/\./g)||[]).length);

  // console.log('xxxx', JSON.stringify(root,undefined,2));
  ...
  if (!options.short) {
    if (data.istemplate || data.ishelper) {
      ...
      if (_.contains(["function", "class"], data.kind)) {
        ...
        // only show top level param names
        paramNames = _.reject(paramNames, s => s.includes('.'));
